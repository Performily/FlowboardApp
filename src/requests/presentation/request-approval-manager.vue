<template>
  <div class="p-4">
    ...
    <!-- Subtítulo -->
    <div class="mb-3">
      <span class="text-lg text-600 font-medium">Lista principal</span>
    </div>
    ...
    >
      ...
      <pv-column field="status" header="Estado" style="width: 130px;">
        <template #body="{ data }">
          <pv-tag
            :value="data.status"
            :severity="
      ...
      <pv-column field="type"         header="Tipo de solicitud" />
      <pv-column field="title"        header="Título" />
      <pv-column field="employeeName" header="Colaborador" />
      <pv-column field="startDate"    header="Fecha de solicitud" />
    ...
    >
      <div class="flex gap-4 mb-3">
        <div class="flex flex-column gap-2 flex-1">
          <div><span class="font-medium">Título</span><p class="m-0 text-600">{{ selected?.title }}</p></div>
          <div><span class="font-medium">Tipo de solicitud</span><p class="m-0 text-600">{{ selected?.type }}</p></div>
          <div><span class="font-medium">Fecha de solicitud</span><p class="m-0 text-600">{{ selected?.startDate }}</p></div>
          <div><span class="font-medium">Motivo</span><p class="m-0 text-600">—</p></div>
        </div>
      ...
      <div class="flex flex-column gap-3">
        <div class="flex flex-column gap-1">
          <label class="font-medium text-sm">Comentarios de colaborador</label>
          <pv-textarea v-model="approveForm.collaboratorComments" rows="3" class="w-full" />
        </div>
        <div class="flex flex-column gap-1">
          <label class="font-medium text-sm">Otros detalles</label>
          <pv-textarea v-model="approveForm.otherDetails" rows="3" class="w-full" />
        </div>
        <p class="font-medium text-sm m-0">¿Desea confirmar la aprobación de esta solicitud?</p>
      </div>

      <template #footer>
        <pv-button label="Confirmar Aprobación" style="background-color:#3b4cb8; border-color:#3b4cb8;" @click="confirmApprove" :loading="store.loading" />
        <pv-button label="Cancelar" severity="secondary" text @click="approveVisible = false" />
      </template>
    </pv-dialog>
    ...
    >
      ...
      <div class="flex flex-column gap-3">
        ...
        <div class="flex flex-column gap-1">
          <label class="font-medium text-sm">Indique el motivo del rechazo: *</label>
          <pv-textarea
            v-model="rejectForm.rejectionReason"
            rows="3"
            class="w-full"
            placeholder="Debe ingresar un motivo para rechazar la solicitud"
          />
        </div>
      </div>

      <template #footer>
        <pv-button label="Confirmar Rechazo" severity="danger" @click="confirmReject" :loading="store.loading" />
        <pv-button label="Cancelar" severity="secondary" text @click="rejectVisible = false" />
      </template>
    </pv-dialog>
  </div>
</template>
